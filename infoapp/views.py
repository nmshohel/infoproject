...
def pbs_info_berc_form(request):
    ...
    if request.method == 'POST':
        ...
        PbsInfoBERC.objects.create(pbs_code=user_pbs_info.pbs_code, management_code=user_pbs_info.management_code, complain_nos=comp_nos,
                                   complain_solve=comp_solve, complain_unsolve_case=com_unsolve_case, complain_remark=com_remarks, month=month, year=year)
...
def new_online_connection(request):
    if request.method == 'POST':
        ...
        NewOnlineConnection.objects.create(pbs_code=user_pbs_info.pbs_code, management_code=user_pbs_info.management_code, total_app=u_total_app,
                                           total_solve_app=u_total_solve_app, cause_of_unsolve=u_cause_of_unsolve, remark=u_remark, month=month, year=year)
...
def industry_comercial_consumer_dc_rc(request):
    if request.method == 'POST':
        ...
        IndustryCommercialDcRc.objects.create(pbs_code=user_pbs_info.pbs_code, management_code=user_pbs_info.management_code, total_dc_app=u_total_dc_app,
                                              complete_rc_app=u_complete_rc_app, illegal_app_for_dc=u_illegal_app_for_dc, total_app_get_for_rc=u_total_app_get_for_rc, month=month, year=year)
...
def dc_consumer(request):
    if request.method == 'POST':
        ...
        DcConsumerInfo.objects.create(pbs_code=user_pbs_info.pbs_code, management_code=user_pbs_info.management_code, domestic=domestic, industry=industry, commercial=commercial,
                                      irrigation=irrigation, month=month, year=year, govt_institute=govt_institute, other=other, total_amount=total_amount, taken_action=taken_action)
...
def connected_consumer(request):
    if request.method == 'POST':
        ...
        total_amount = request.POST.get('total_amount')
        taken_action = request.POST.get('taken_action')
        ConnectedConsumerInfo.objects.create(pbs_code=user_pbs_info.pbs_code, management_code=user_pbs_info.management_code, domestic=domestic, industry=industry, commercial=commercial,
                                             irrigation=irrigation, month=month, year=year, govt_institute=govt_institute, other=other, total_amount=total_amount, taken_action=taken_action)
...
def domestic_connection_seven_day(request):
    if request.method == 'POST':
        ...
        con_without_seven_day_app = request.POST.get(
            'con_without_seven_day_app')
        process_seven_day_app = request.POST.get('process_seven_day_app')
        reason_not_con_of_seven_day = request.POST.get(
            'reason_not_con_of_seven_day')
        remarks = request.POST.get('remarks')
        DomesticConnectionSevenDay.objects.create(pbs_code=user_pbs_info.pbs_code, management_code=user_pbs_info.management_code, total_app=total_app, con_within_seven_day_app=con_within_seven_day_app,
                                                  con_without_seven_day_app=con_without_seven_day_app, process_seven_day_app=process_seven_day_app, month=month, year=year, reason_not_con_of_seven_day=reason_not_con_of_seven_day, remarks=remarks)
...
def monthly_coordination_meeting_info(request):
    if request.method == 'POST':
        ...
        previous_month_unsolve_objection = request.POST.get(
            'previous_month_unsolve_objection')
        current_month_unsolve_objection = request.POST.get(
            'current_month_unsolve_objection')
        total_objection = request.POST.get('total_objection')
        current_month_solve_objection = request.POST.get(
            'current_month_solve_objection')
        unsolve_objection = request.POST.get('unsolve_objection')
        remarks = request.POST.get('remarks')
        MonthlyCoordinationMeetingInfo.objects.create(pbs_code=user_pbs_info.pbs_code, management_code=user_pbs_info.management_code, consumer_meeting_nos=consumer_meeting_nos, previous_month_unsolve_objection=previous_month_unsolve_objection,
                                                      current_month_unsolve_objection=current_month_unsolve_objection, total_objection=total_objection, month=month, year=year, current_month_solve_objection=current_month_solve_objection, remarks=remarks, unsolve_objection=unsolve_objection)
...
def necessary_action_against_accident(request):
    if request.method == 'POST':
        ...
        install_pole_for_unshift_cable = request.POST.get(
            'install_pole_for_unshift_cable')
        cover_cable_for_non_install_pole = request.POST.get(
            'cover_cable_for_non_install_pole')
        ...
        pdb_risk_pole_check_change = request.POST.get(
            'pdb_risk_pole_check_change')
        electric_line_maintainance_by_instraction = request.POST.get(
            'electric_line_maintainance_by_instraction')
        NecessaryActionAgainstAccident.objects.create(pbs_code=user_pbs_info.pbs_code, management_code=user_pbs_info.management_code, shift_cable=shift_cable, install_pole_for_unshift_cable=install_pole_for_unshift_cable, cover_cable_for_non_install_pole=cover_cable_for_non_install_pole,
                                                      span_length_sag_visit=span_length_sag_visit, month=month, year=year, risk_unfit_pole_change=risk_unfit_pole_change, pdb_risk_pole_check_change=pdb_risk_pole_check_change, electric_line_maintainance_by_instraction=electric_line_maintainance_by_instraction)
...
def m_berc_info(request):
    ...
    user_pbs_management_info = PbsInfo.objects.filter(
        management_code=user_management_info.management_code)
    my_data = PbsInfoBERC.objects.filter(
        management_code=user_management_info.management_code)
    context = {'my_data': my_data,
               'user_pbs_management_info': user_pbs_management_info}
...
def m_new_con_info(request):
    ...
    user_pbs_management_info = PbsInfo.objects.filter(
        management_code=user_management_info.management_code)
    my_data = NewOnlineConnection.objects.filter(
        management_code=user_management_info.management_code)
    context = {'my_data': my_data,
               'user_pbs_management_info': user_pbs_management_info}
...
def m_industry_commercial_dc_rc(request):
    ...
    user_pbs_management_info = PbsInfo.objects.filter(
        management_code=user_management_info.management_code)
    my_data = IndustryCommercialDcRc.objects.filter(
        management_code=user_management_info.management_code)
    context = {'my_data': my_data,
               'user_pbs_management_info': user_pbs_management_info}
...
def m_dc_consumer(request):
    ...
    user_pbs_management_info = PbsInfo.objects.filter(
        management_code=user_management_info.management_code)
    my_data = DcConsumerInfo.objects.filter(
        management_code=user_management_info.management_code)
    context = {'my_data': my_data,
               'user_pbs_management_info': user_pbs_management_info}
...
def m_connected_consumer(request):
    ...
    user_pbs_management_info = PbsInfo.objects.filter(
        management_code=user_management_info.management_code)
    my_data = ConnectedConsumerInfo.objects.filter(
        management_code=user_management_info.management_code)
...
def m_domestic_connection_seven_day(request):
    ...
    user_pbs_management_info = PbsInfo.objects.filter(
        management_code=user_management_info.management_code)
    my_data = DomesticConnectionSevenDay.objects.filter(
        management_code=user_management_info.management_code)
    context = {'my_data': my_data,
               'user_pbs_management_info': user_pbs_management_info}
...
def m_monthly_coordination_meeting(request):
    ...
    user_pbs_management_info = PbsInfo.objects.filter(
        management_code=user_management_info.management_code)
    my_data = MonthlyCoordinationMeetingInfo.objects.filter(
        management_code=user_management_info.management_code)
    context = {'my_data': my_data,
               'user_pbs_management_info': user_pbs_management_info}
...
def m_actin_against_accident(request):
    ...
    user_pbs_management_info = PbsInfo.objects.filter(
        management_code=user_management_info.management_code)
    my_data = NecessaryActionAgainstAccident.objects.filter(
        management_code=user_management_info.management_code)
    context = {'my_data': my_data,
               'user_pbs_management_info': user_pbs_management_info}
...
def user_register(request):
    if request.method == 'POST':
        ...
        user = User.objects.create_user(
            first_name=u_first_name, last_name=u_last_name, username=user_name, password=user_pass, email=u_email)
        pbsinfo_user = PbsInfo.objects.create(user=user, management_code=management_id, pbs_name=u_pbs_name,
                                              pbs_name_benglai=u_pbs_name_benglai, pbs_code=u_pbs_code, address=u_address, mobile_no=u_mob_no)
        register_form = UserForm()
        pbs_info_register_form = PBSInfoForm()
        message = "User Successfully Added"
        context = {'register_form': register_form,
                   'pbs_info_register_form': pbs_info_register_form, 'message': message}
        ...
        pbs_info_register_form = PBSInfoForm()
        context = {'register_form': register_form,
                   'pbs_info_register_form': pbs_info_register_form}
        return render(request, 'user_register.html', context)
